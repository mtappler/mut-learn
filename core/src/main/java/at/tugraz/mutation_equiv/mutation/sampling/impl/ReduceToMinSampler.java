 ...
 *
 */
public class ReduceToMinSampler implements MutantSamplingStrategy {
	private String kindPattern = null;
	private Random randGen;

	public ReduceToMinSampler(Random randGen, String kindPattern) {
	...
	public MutantSample sample(MutantSample original) {
		if (kindInChildren(original)) {
			...
			// List<MutantSample> sortedChildren = matchingChildren.stream()
			// .sorted((s1, s2) -> s1.getMutants().size() -
			// s2.getMutants().size()).collect(Collectors.toList());
			final int min = matchingChildren.stream().mapToInt(s -> s.getMutants().size()).min().getAsInt(); // sortedChildren.get(0).getMutants().size();
			List<MutantProducer> sampledMatching = matchingChildren.stream()
					.flatMap(ms -> sampleList(ms.getMutants(), min).stream()).collect(Collectors.toList());
