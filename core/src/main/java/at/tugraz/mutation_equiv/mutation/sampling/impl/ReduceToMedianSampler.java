 ...
 * calculates the minimum number of mutants in the same kind at the same level
 ...
 *
 */
public class ReduceToMedianSampler implements MutantSamplingStrategy {
	private String kindPattern = null;
	private Random randGen;

	public ReduceToMedianSampler(Random randGen, String kindPattern) {
	...
	public MutantSample sample(MutantSample original) {
		if (kindInChildren(original)) {
			...
			List<Integer> sortedSizes = matchingChildren.stream().map(s -> s.getMutants().size()).sorted().collect(Collectors.toList());
			int median = 0;
			if(sortedSizes.size() % 2 == 1){
				median = sortedSizes.get((sortedSizes.size() - 1)/2);
			} else {
				median = (sortedSizes.get(sortedSizes.size()/2) + 
						sortedSizes.get(sortedSizes.size()/2-1)) /2 ;				
			}

			final int medianClosure = median;
			List<MutantProducer> sampledMatching = matchingChildren.stream()
					.flatMap(ms -> sampleList(ms.getMutants(), medianClosure).stream()).collect(Collectors.toList());
	...
	public String description() {
		return String.format("reduce-to-min(kind=%s)", kindPattern);
	}
