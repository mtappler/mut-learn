 ...
 * mutants from a given kind. 
 * Since it does provide a lot of benefits according to experience, it's
 * not used in the evaluation. 
 * 
 *  However, it works as follows, it looks for a specific kind of samples
 *  and calculates the average number of mutants in the same kind at the same level in
 *  the hierarchy. It then selects a subset of mutants from each samples of the 
 *  specified kind which may be as large as the average sample size.
 * 
...
public class ReduceToMeanSampler implements MutantSamplingStrategy{
	...
	public MutantSample sample(MutantSample original) {
		if(kindInChildren(original)) {
			...
			List<MutantSample>
				matchingChildren = original.getChildren().stream().filter(s -> s.getKind().contains(kindPattern))
			...
			List<MutantProducer> sampledMatching = matchingChildren.stream()
					.flatMap(ms -> sampleList(ms.getMutants(),mean).stream())
					.collect(Collectors.toList());
	...
	private boolean kindInChildren(MutantSample original){
		return original.getChildren().stream().filter(s -> s.getKind().contains(kindPattern)).findAny().isPresent();
	}
}
