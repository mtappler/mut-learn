...
/**
 * The class that implements the equivalence oracle, i.e. the basic data flow, creating 
 * test cases of which a subset is selected. It also implements basic logging, can be configured
 * flexibly, but delegates the actual test-case generation and selection to other 
 * classes.
 * 
 * Although it can be used for different generation strategies, it is mainly intended to be
 * used with mutation so the mutation operators and sampling strategies can configured 
 * in this class. 
 * 
 * Furthermore, it can be configured to reuse tests (<code>reuseRemaining</code>) between 
 * rounds of learning. Hence, if generation and selection are expensive, the execution of 
 * these can be avoided to some extent. Additionally, tests executed on the SUL can be stored
 * in order to account for them in test-case selection in later rounds.
 * 
 * @author Martin Tappler
 *
 * @param <T> Type of test case which is used the test-case selector
 */
public class RandomCoverageSelectionEQOracle<T extends TestCase> implements MealyEquivalenceOracle<Symbol, String> {
	
	private SUL<Symbol,String> sul;
	...
	public RandomCoverageSelectionEQOracle(SUL<Symbol,String> sul, 
	...
	public DefaultQuery<Symbol, Word<String>> findCounterExample(MealyMachine<?, Symbol, ?, String> hypothesis,
		...
		if(reuseRemaining && remainingTests != null){
			...
			FastMealy<Symbol, String> hypoCasted = MutationOperator.copyMealyMachine(hypothesis,alphabet , Collections.emptyList()).getLeft();
			...
			// currently one mutants may be in both the set of mutants for generation and in 
			// the set for selection which not be a problem as mutants (MutantProducer) are effectively immutable
			...
			List<List<Symbol>> tests = createTests((MealyMachine<Object,Symbol,?,String>)
					((MealyMachine<?,Symbol,?,String>)hypoCasted),sampledGenerationMutants.getMutants());
		...
		while(bestTests.hasNext()){
			...
			nrExecutedTests ++;
			Optional<DefaultQuery<Symbol, Word<String>>> differingQuery = 
					differingSULOutput.map(actualSULOutput -> {
